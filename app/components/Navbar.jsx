...
const Navbar = () => {
    ...
    return (
        <>
            <div className='flex justify-between items-center mt-0 bg-black w-full px-6 py-4 rounded-b-2xl'>
                <h1 className='text-lg font-extrabold  text-yellow-500 rounded-3xl m-1 border border-solid border-slate-900 p-1 cursor-pointer  '>RapidReads</h1>
                {
                    !userData?.isAuth ? (
                        <button onClick={signIn} className='px-5 py-2 rounded-2xl  bg-green-400 text-xs md:text-sm  hover:translate-y-2 hover:transition-all hover:duration-150'>Login</button>
                    ) : (
                        <div className='flex items-center'>
                            <Image src={userData?.user?.photoURL} alt='dp' width={30} height={30} className='rounded-full mr-2' />
                            <button onClick={logout} className=' px-5 py-2 rounded-2xl  bg-green-400 text-xs md:text-sm  hover:translate-y-2 hover:transition-all hover:duration-150'>Logout</button>
                        </div>
                    )
                }
            </div>
